...
(function () {
  ...
  // Format as YYYY-MM-DD using local date to avoid UTC timezone offset shifting the date
  ...
  // Is this date in the middle of a booking (strictly after check-in, before check-out)?
  // A date that is only a booking's check-in day is a changeover day and can be used as
  // a checkout date for the preceding stay.
  function isMidStay(date, roomKey) {
    const bookings = (typeof BOOKINGS !== 'undefined') ? (BOOKINGS[roomKey] || []) : [];
    return bookings.some(b => {
      const from = fromYMD(b.from);
      const to   = fromYMD(b.to);
  ...
  function _buildCell(date, today) {
    ...
    // A changeover day is one that is the check-in of an existing booking but NOT a
    // mid-stay night — guests leave and new guests arrive on the same day.
    // When the user is selecting a checkout date, a changeover day after their check-in
    // is valid (they leave before the next guests arrive).
    const isChangeoverDay = isBooked && !isMidStay(date, state.roomKey);
    const canBeCheckout   = isChangeoverDay &&
                            state.selecting === 'out' &&
                            state.checkIn &&
                            date > state.checkIn;
    ...
    } else if (isBooked && !canBeCheckout) {
  ...
  function _buildLegend() {
    ...
    const items = [
      ...
      { cls: 'cal-past',    label: t.legend_past     },
      { cls: 'cal-selected-in', label: t.legend_selected },
    ];

    items.forEach(item => {
      const wrap = document.createElement('div');
      wrap.className = 'cal-legend-item';
      const swatch = document.createElement('span');
      swatch.className = 'cal-legend-swatch cal-cell ' + item.cls;
